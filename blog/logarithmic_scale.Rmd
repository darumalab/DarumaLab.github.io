...
```{r}
# 指数関数的成長: 10年後の大麦の数はいくつだろう。
time <- 1:10 # the time period,
r <- 0.5 # the growth rate.
a <- 10 # the initial amount
y <- a * (1 + r) ^ time # (1+r) is the growth factor
# create an index vector with seq_along
plt <- qplot(seq_along(y), y, xlab = "年数", ylab = "大麦の数")
plt + scale_y_continuous(breaks = seq(0, 1000, by = 100)) +
...
リスは銀行に渡された線形目盛のチャートをながめて幸せそうだ。しかし5年目から10年目の粒の増え方に比べて、1年目から5年目までの大麦の増え方が分かりづらいと思った。リスは銀行に新しいチャートを用意させることにした。「5年目までの大麦の増え方をもっと分かりやすく見せてくれ」
...
plt + scale_y_continuous(breaks = seq(1, 6.5, by = 0.5)) +
  scale_x_continuous(breaks = seq(0, 10, by = 1))
```
*チャート2:大麦の数と貯蓄年数*
...
大麦は毎年同じ増加率を達成していた。リスは銀行に渡された対数目盛のチャートを見てとても満足した。一方の軸が対数目盛になっているグラフを片対数(semi-log)チャートと呼ぶ。^[[片対数グラフ](https://ja.wikipedia.org/wiki/%E7%89%87%E5%AF%BE%E6%95%B0%E3%82%B0%E3%83%A9%E3%83%95)]片対数チャートを見ると増加傾向は毎年50%を保っていることが分かる。

1年目から5年目まではわずかな増加だった大麦の数量は、5年目から10年目にかけて急激に膨れ上がった。そのせいで、1年目から5年目までの情報がはっきりしないものになっていた。チャート2ではデータの全体像を正確に捉えることができている。
